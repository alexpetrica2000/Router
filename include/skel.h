...
typedef struct AVLNode{
	rtable_entry entry; // entryul din rtable in care am prefix mask nexthop 
						// si interface
	uint32_t index;     // index este valoarea dupa care se fac comparatiile
						// in acest caz prefixul
	int height;
	struct AVLNode *l;
	struct AVLNode *r;
	struct AVLNode *p;
...
typedef struct AVLTree{
	long size;
	AVLNode* root;
	uint32_t (*comp)(uint32_t a, uint32_t b);
}AVLTree;
	...
	void delFixup(AVLTree* tree,AVLNode* x);

	void AVLDelNode(struct AVLTree* tree, uint32_t index);

	AVLTree* avlCreateTree(uint32_t (*comp) (uint32_t,uint32_t));

	void printHeightsAndElems_(AVLTree* tree, AVLNode* node);

	void printHeightsAndElems(AVLTree* tree);
